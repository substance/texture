...
class Validator {
  ...
  constructor(data) {
    this.elementNames = data.tagNames
    this.elementIds = data.tagNames.reduce((m,k,idx)=>{m[k]=idx;return m}, {})
    let dfas = {}
    const EPSILON = this.getId('EPSILON')
    forEach(data.elements, (data, name) => {
      dfas[name] = new DFA(data, EPSILON)
    })
    this.dfas = dfas
    this.errors = []
  }
