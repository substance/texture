...
function parseAttribute(state, el) {
  ...
  let name = attributes.ref || attributes.name
  let params = {}
  if (attributes['use'] === 'optional') {
  ...
  if (attributes['type']) {
    params.valueType = attributes.type
  }
  // TODO: can we cast this to the correct type?
  if (attributes.hasOwnProperty('default')) {
    params.defaultValue = attributes['default']
  }
  if (attributes.hasOwnProperty('fixed')) {
    params.fixedValue = attributes['fixed']
  }
...
/*
  ...
  - inside complexTypes there are empty choice elements:
  ...
  - inside groups there are empty choice elements, additionally these are often ref'd with *-cardinality:
    anyURI, oasis.prefix, city-elements, contrib-id-model,
    institution-id-model, tbl.table-titles.mdl,
    postal-code-elements, rt-elements, state-elements
    => seems to be a way to allow for #PCDATA
