...
function testAnnotationToggle (t, annoType) {
  ...
  setSelection(editor, 'p-2.content', 290, 300)
  ...
  t.equal(offset, '290', 'The data-offset property must be equal to begining of the selection')
  let length = anno.el.getAttribute('data-length')
  t.equal(length, '10', 'The data-length property must be equal to the length of the selection')
  let text = anno.text()
  ...
  setCursor(editor, 'p-2.content', 295)
  t.equal(isToolActive(articlePanel, annoType), true, 'Tool must be active')
  annoToogle.find('button').click()
...
// Strong, em, link, subs, supers, monospace

// test('Strong: toggle annotation', t => {
//   let { app } = setupTestApp(t)
//   let articlePanel = app.find('.sc-article-panel')
//   let annoToogle = articlePanel.find('.sc-toggle-tool.sm-bold')
//   let editor = articlePanel.find('.sc-manuscript-editor')

//   // Set the cursor and check if tool is active
//   setCursor(editor, 'p-2.content', 290)
//   t.equal(isToolActive(articlePanel, '.sm-bold'), false, 'Tool must be disabled')
//   // Set the selection and check if tool is active
//   setSelection(editor, 'p-2.content', 290, 300)
//   t.equal(isToolActive(articlePanel, '.sm-bold'), true, 'Tool must be active')
//   // Toggle the tool and check if an annotation appeared
//   annoToogle.find('button').click()
//   let anno = editor.find('[data-path="p-2.content"] .sc-bold')
//   t.notNil(anno, 'There should be an annotation')
//   let offset = anno.el.getAttribute('data-offset')
//   t.equal(offset, '290', 'The data-offset property must be equal to begining of the selection')
//   let length = anno.el.getAttribute('data-length')
